...
xnm <- c('Al', 'As', 'Ca','Cr','Cu', 'Fe', 'Pb', 'Mg', 'Mn','Hg','Ni','Se','Zn')

## create R object with all covariates
covars = c('CE_AGE', 'EDU', 'EN_FORMERSMOKER', 'passivesmoke','CdBinary','headinjury')

bkmr.data <- na.omit(data[,c(xnm, covars, 'CE_BARS_DST_FORWARD_CNT')])
z.data = as.matrix(bkmr.data[,c(xnm)])
x.data = as.matrix(bkmr.data[,c(covars)])
y.data= bkmr.data$CE_BARS_DST_FORWARD_CNT
...
save.image(file = "DSTforwardall_results.RData")
