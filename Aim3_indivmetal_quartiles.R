...
data$gripAVG <- as.numeric(data$gripAVG)
data$CE_V4B_TEST_A_TIME <- as.numeric(data$CE_V4B_TEST_A_TIME)
data$CE_V4B_TEST_B_TIME <- as.numeric(data$CE_V4B_TEST_B_TIME)
data$poststabOPEN <- as.numeric(data$poststabOPEN)
data$poststabCLOSED <- as.numeric(data$poststabCLOSED)
data$vibrototal <- as.numeric(data$vibrototal)
...
mod1 <- lm(CE_BARS_CPT_FA_FRACTION ~ PbQuartile + CE_AGE + EDU + EN_FORMERSMOKER + headinjuryever + drinklot + marital, data = data)
summary(mod1)
confint(mod1)
plot(mod1)
AIC(mod1)
...
mod1<- lm( CE_BARS_CPT_FA_FRACTION~ MoBinary + CE_AGE + Batch + EDU + EN_FORMERSMOKER + passivesmoke+ headinjuryever + drinklot, data = data)
...
mod1<- lm( CE_BARS_CPT_FA_FRACTION~ SbTertile + CE_AGE + Batch + EDU + EN_FORMERSMOKER + passivesmoke+ headinjuryever + drinklot, data = data)
...
mod1<- lm( CE_BARS_CPT_FA_FRACTION~ AsQuartile + CE_AGE + Batch + EDU + EN_FORMERSMOKER + passivesmoke+ headinjuryever + drinklot + marital, data = data)
...
mod1<- lm( CE_BARS_CPT_FA_FRACTION~ PbQuartile + CE_AGE + Batch + EDU + EN_FORMERSMOKER + passivesmoke+ headinjuryever + drinklot + marital, data = data)
...
library(patchwork)
 CE_BARS_CPT_FA_FRACTION<-(plotCo| plotCd| plotMo)/
(plotSb| plotV | plotAl)/ 
(plotAs| plotCa| plotCr)/
(plotCu| plotFe| plotPb)/
(plotMg| plotMn| plotHg)/
(plotNi| plotSe| plotZn)
