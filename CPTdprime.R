...
p3 <- ggplot(overall.estimates.5chain, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) + 
  ...
  theme_bw() + scale_y_continuous(limits=c(-0.46,0.66),breaks=c( -0.4, -0.2, 0, 0.2, 0.4, 0.6))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
...
save.image(file = "scaleall_CPTdprimeall_results.RData")



CuCr<- pred.resp.bivar.levels.data %>%
  filter(variable1 == "Cu", variable2 == "Cr")
CrCu<- pred.resp.bivar.levels.data %>%
  filter(variable1 == "Cr", variable2 == "Cu")

p<- ggplot(CrCu, aes(z1, est))+ geom_smooth(aes(col = quantile), stat = "identity")+
  geom_hline(yintercept=0, linetype= "dashed") + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  ylab("Outcome") +
  xlab("Exposure")+ scale_y_continuous(limits=c(-0.5,0.4),breaks=c( -0.4, -0.2, 0, 0.2, 0.4))+ scale_x_continuous(limits=c(-4,4),breaks=c( -4, -2, 0, 2, 4))
p
