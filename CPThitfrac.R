...
data <- data %>% mutate(CPTcr1 = case_when(CE_BARS_CPT_CR_FRACTION < 0.80 ~ '0',
                                           CE_BARS_CPT_CR_FRACTION >= 0.8 ~ '1')) 
data <- data %>% mutate(CPThit1 = case_when(CE_BARS_CPT_HIT_FRACTION < 0.80 ~ '0',
                                            CE_BARS_CPT_HIT_FRACTION >= 0.8 ~ '1'))  
data$CPTcr1 <- as.numeric(data$CPTcr1)
data$CPThit1 <- as.numeric(data$CPThit1) 

## create R object with all exposures for regression models
xnm <- c('As','Cr', 'Cu', 'Pb', 'Mg', 'Mn')

## create R object with all covariates
covars = c('CE_AGE', 'CE_C1', 'EN_FORMERSMOKER','CdBinary', 'marital','drinklot','CE_BMI')

bkmr.data <- na.omit(data[,c(xnm, covars, 'CPThit1')])
z.data = as.matrix(bkmr.data[,c(xnm)])
x.data = as.matrix(bkmr.data[,c(covars)])
y.data= bkmr.data$CPThit1

set.seed(1234)
system.time(fitkm.model1 <- kmbayes_parallel(nchains=5, y = y.data, Z = z.data, X = x.data, iter = 40000, verbose = FALSE, varsel = TRUE, family = "binomial", control.params = list(r.jump2=0.5), est.h = TRUE))
...
pred.resp.bivar.levels.data <- PredictorResponseBivarLevels(pred.resp.df = pred.resp.bivar.data, Z=z.data, qs=c(0.1, 0.25, 0.50, 0.75, 0.90))
...
save.image(file = "CPThitfracall_results.RData")
